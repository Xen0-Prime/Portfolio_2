...
// Options MQTT sécurisées
const MQTT_OPTIONS = {
    host: 'broker.hivemq.com',
    port: 8884,
    protocol: 'wss', // WebSocket Secure (pour navigateur) ou 'mqtts' pour MQTT over TLS
    username: '', // Remplir si authentification requise
    password: '', // Remplir si authentification requise
    // Options TLS additionnelles
    rejectUnauthorized: true, // Vérifie le certificat du serveur
...
const SUPABASE_CONFIG = {
    ...
    key: 'sb_publishable_gTb7gRITTA9GiTmTpbHpdg_Y2tpB7NC',
    table: 'iot_data'
};
...
function connect() {
    ...
    // Utiliser les options sécurisées ou l'URL simple selon la configuration
    if (MQTT_OPTIONS.username && MQTT_OPTIONS.password) {
        // Connexion avec authentification et options complètes
        client = mqtt.connect(MQTT_OPTIONS);
        addLog('🔒 Connexion sécurisée avec authentification', '');
    } else {
        // Connexion simple avec URL (fallback)
