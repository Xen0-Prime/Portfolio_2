...
const SUPABASE_CONFIG = {
    ...
    key: 'sb_publishable_gTb7gRITTA9GiTmTpbHpdg_Y2tpB7NC',            // À MODIFIER
    table: 'iot_data'
};
...
function updateHistoryChart() {
    if (!chartInstance || historicalData.length === 0) return;
    ...
    chartInstance.data.datasets[2].data = pressData;
...
function exportToCSV() {
    ...
    const headers = ['Timestamp', 'Device ID', 'Temperature', 'Humidity', 'Pressure'];
    const csv = [
        headers.join(','),
...
function connect() {
    ...
    client.on('message', (topic, message) => {
        ...
        try {
            ...
            if (topic === TOPICS.temperature) updateValue('temperature', value);
            else if (topic === TOPICS.pression) updateValue('pression', value);
            else if (topic === TOPICS.humidite) updateValue('humidite', value);
            else if (topic === TOPICS.position) updateValue('position', value);
        } catch (e) {
